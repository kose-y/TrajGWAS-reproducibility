...
@time nm = trajgwas(@formula(std_dbp ~ 1 + SEX + std_age + std_age_sq + std_age&SEX +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_dbp ~ 1 + std_age),
    @formula(std_dbp ~ 1 + SEX + std_age + std_age_sq + std_age&SEX),
    :IID,
    bp_data,
    nothing;
    nullfile="dbp.run5_final.all.null.txt",
    solver=solver,
    runs=10
)
...
println(nm)
using Serialization
open("fittednullmodel.dbp.run5_final.all.jls", "w") do io
